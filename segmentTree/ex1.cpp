...
void update(int node, int start, int end, int i, int val){
    ...
    int mid = (start+end)/2;
    if(i <= mid) update(2*node+1, start, mid, i, val );
    else update(2* node+2, mid+1, end, i, val );
    tree[node] = min(tree[2*node+1], tree[2*node+2]);
...
int main() {
    ...
    //ex-> arr = [1,3,2,4,5,2,3]
            //    0 1 2 3 4 5 6                            
    cout<< quary(0,0,n-1, 2,4)<<endl; //2
    cout<< quary(0,0,n-1, 3,5)<<endl; // 2
    cout<< quary(0,0,n-1, 0,4)<<endl; // 1
    arr[2] = 0;
    //ex-> arr = [1,3,0,4,5,2,3]
    //    0 1 2 3 4 5 6    
    update(0,0,n-1,2,0);
    cout<< quary(0,0,n-1, 2,4)<<endl; // 0
    arr[4] =2;
    //ex-> arr = [1,3,0,4,2,2,3]
    //    0 1 2 3 4 5 6    
    update(0,0,n-1,4,2);
    cout<< quary(0,0,n-1, 2,5)<<endl; //0
    cout<< quary(0,0,n-1, 1,4)<<endl; // 0
    cout<< quary(0,0,n-1,3,4)<<endl; // 2
